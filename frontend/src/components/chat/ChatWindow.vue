...
const canSendMessage = computed(() => {
  return socket.isConnected && !loading.value;
});
  ...
  socket.onTypingStatus((data) => {
    if (data.chatId === props.chatId) {
      typingUsers.value = data.typingUsers
        .filter(id => id !== user.value._id)
        .map(id => 'Someone'); // Replace with actual user names if available
    }
  });
...
const sendMessage = async () => {
  ...
  try {
    ...
    socket.sendMessage(props.chatId, messageText.value);
    messageText.value = '';
